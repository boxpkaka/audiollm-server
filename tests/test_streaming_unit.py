...
# ---------------------------------------------------------------------------
# TS-ASR dual-channel gate (TS-ASR + Qwen3-ASR presence gate)
# ---------------------------------------------------------------------------


def _make_tsasr_engine_with_enrollment():
    """Helper: build TsAsrTaskEngine with a stubbed enrollment so handlers run."""
    from backend.tasks.ts_asr import TsAsrTaskEngine
    from backend.tsasr.enrollment import EnrollmentAudio

    engine = TsAsrTaskEngine()
    engine._enrollment = EnrollmentAudio(
...
def _patch_dual(monkeypatch, *, ts_result, sec_result):
    """Monkeypatch the two backends invoked by _dual_infer.

    Each ``*_result`` is either a dict (returned by the coroutine) or an
    Exception subclass instance (raised by the coroutine).
    """

    async def _ts(*args, **kwargs):
    ...
    monkeypatch.setattr("backend.tasks.ts_asr.query_tsasr_model", _ts)
    monkeypatch.setattr("backend.tasks.ts_asr.query_audio_model_secondary", _sec)


@pytest.mark.asyncio
async def test_tsasr_dual_infer_both_non_empty_returns_both_texts(monkeypatch):
    cfg = load_config()
    ctx = SessionContext(cfg=cfg, language="zh", send_json=lambda _: True)
    _patch_dual(
        monkeypatch,
        ts_result={"transcription": "hello world", "detected_language": "English"},
        sec_result={"transcription": "hi"},
    )
    engine = _make_tsasr_engine_with_enrollment()
    text, sec_text, lang = await engine._dual_infer("dummy_b64", ctx)
...
@pytest.mark.asyncio
async def test_tsasr_dual_infer_both_empty_returns_empty_texts(monkeypatch):
    cfg = load_config()
    ctx = SessionContext(cfg=cfg, language="zh", send_json=lambda _: True)
    _patch_dual(
        monkeypatch,
        ts_result={"transcription": ""},
        sec_result={"transcription": ""},
    )
    engine = _make_tsasr_engine_with_enrollment()
    text, sec_text, _ = await engine._dual_infer("dummy_b64", ctx)
    assert text == ""
    assert sec_text == ""


@pytest.mark.asyncio
async def test_tsasr_dual_infer_default_surfaces_both_paths(monkeypatch):
    """Default behavior (gate off) surfaces both transcripts independently.

    Either path can be empty without affecting the other — the dual-display
    UX wants to render whichever side has content rather than suppress the
    whole bubble. This is the inverse of the gate behavior tested below.
    """
    cfg = load_config()
    ctx = SessionContext(cfg=cfg, language="zh", send_json=lambda _: True)
    _patch_dual(
        monkeypatch,
        ts_result={"transcription": ""},
        sec_result={"transcription": "interferer says hi"},
    )
    engine = _make_tsasr_engine_with_enrollment()
    text, sec_text, _ = await engine._dual_infer("dummy_b64", ctx)
    assert text == ""
    assert sec_text == "interferer says hi"


@pytest.mark.asyncio
async def test_tsasr_dual_infer_gate_ts_empty_suppresses(monkeypatch):
    """With the optional silence gate ON, an empty TS-ASR side zeros both."""
    cfg = load_config().override(tsasr_enable_secondary_gate=True)
    ctx = SessionContext(cfg=cfg, language="zh", send_json=lambda _: True)
    _patch_dual(
        monkeypatch,
        ts_result={"transcription": ""},
...
@pytest.mark.asyncio
async def test_tsasr_dual_infer_gate_secondary_empty_suppresses(monkeypatch):
    """With the gate ON, an empty Qwen3 side zeros both (TS-ASR hallucination guard)."""
    cfg = load_config().override(tsasr_enable_secondary_gate=True)
    ctx = SessionContext(cfg=cfg, language="zh", send_json=lambda _: True)
    _patch_dual(
        monkeypatch,
        ts_result={"transcription": "phantom"},
        sec_result={"transcription": ""},
    )
    engine = _make_tsasr_engine_with_enrollment()
    text, sec_text, _ = await engine._dual_infer("dummy_b64", ctx)
...
@pytest.mark.asyncio
async def test_tsasr_dual_infer_ts_raises_returns_empty_primary(monkeypatch):
    """A failing TS-ASR call collapses the primary text to empty.

    The secondary path is preserved so the UI can still render the Qwen3
    row; only the gate (when enabled) would zero both.
    """
    cfg = load_config()
    ctx = SessionContext(cfg=cfg, language="zh", send_json=lambda _: True)
    _patch_dual(
        monkeypatch,
        ts_result=RuntimeError("ts boom"),
        sec_result={"transcription": "hello"},
    )
    engine = _make_tsasr_engine_with_enrollment()
    text, sec_text, _ = await engine._dual_infer("dummy_b64", ctx)
    assert text == ""
    assert sec_text == "hello"


@pytest.mark.asyncio
async def test_tsasr_dual_infer_secondary_raises_returns_empty_secondary(monkeypatch):
    """A failing Qwen3 call collapses the secondary row but keeps TS-ASR text."""
    cfg = load_config()
    ctx = SessionContext(cfg=cfg, language="zh", send_json=lambda _: True)
    _patch_dual(
        monkeypatch,
        ts_result={"transcription": "hello world"},
        sec_result=RuntimeError("qwen boom"),
    )
    engine = _make_tsasr_engine_with_enrollment()
    text, sec_text, _ = await engine._dual_infer("dummy_b64", ctx)
    assert text == "hello world"
...
@pytest.mark.asyncio
async def test_tsasr_dual_infer_show_secondary_off_skips_secondary(monkeypatch):
    """When both flags are off, the secondary path is never invoked."""
    cfg = load_config().override(
        tsasr_show_secondary_text=False,
        tsasr_enable_secondary_gate=False,
    )
    ctx = SessionContext(cfg=cfg, language="zh", send_json=lambda _: True)
    sec_calls = {"n": 0}

    async def _ts(*_args, **_kwargs):
        return {"transcription": "hello", "detected_language": "English"}

    async def _sec(*_args, **_kwargs):
        sec_calls["n"] += 1
        return {"transcription": "hi"}

    monkeypatch.setattr("backend.tasks.ts_asr.query_tsasr_model", _ts)
    monkeypatch.setattr(
        "backend.tasks.ts_asr.query_audio_model_secondary", _sec
    )
