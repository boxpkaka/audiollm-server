...
`POST /api/audio/analyze` 面向离线音频处理场景。调用方上传一段 WAV 音频和可选热词，服务端返回 ASR 识别文本、清洗后的转写文本，以及情感理解结果。情感理解默认同时返回分类标签和文本描述。

热词只用于 ASR 模型原生识别，不会传给文本清洗阶段。文本清洗只负责标点、空格、重复词和明显文本格式问题，不做基于热词的事后替换。
...
| `hotwords` | string | 否 | 逗号分隔热词，如 `挚音科技,张硕`；仅传给 ASR |
