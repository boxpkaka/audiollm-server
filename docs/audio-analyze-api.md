...
热词偏置来自当前 `user_id` 对应的 Triton 用户热词池对音频召回的 top-K 结果，并让少量表单 `hotwords` 临时热词优先进入 ASR prompt（默认最多 8 个，去重后不写入用户池），同时过滤与临时热词精确重复或整词同音（忽略声调）的召回词。`hotwords` 不会传给文本清洗阶段；文本清洗只负责标点、空格、重复词和明显文本格式问题，不做基于热词的事后替换。
