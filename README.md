...
- 可选：Triton 热词召回服务（默认 `http://localhost:10001` / `rag_asr_retrieve`），用于 10 万级全局热词池召回
...
| **热词** | Triton 全局热词池召回 top-K，召回词注入 ASR prompt；1.7B 可走 encoder bypass |
...
| `{"type": "update_hotwords", "hotwords": ["词1", "词2"]}` | 兼容旧客户端；`hotwords` 不再驱动 ASR 偏置，可继续用于携带 `enrollment_id` |
...
- `hotwords` — 兼容旧客户端的字段。新方案不再把会话热词全量拼入 prompt；ASR 偏置来自 Triton 全局热词池对每段音频召回的 top-K 结果
...
ASR 热词偏置来自 `config.yaml -> services.recall` 指向的 Triton 全局热词池。final 段先召回 top-K 热词，再把召回词写入主 ASR prompt；当主模型为 `amphion_asr_1.7b` 且未启用目标说话人时，服务端还会把 Triton 返回的 projector 帧作为 `audio_embeds` 发送给 vLLM，避免重复跑音频 encoder。伪流式 partial 不执行召回、不注入热词、也不走 encoder bypass，只使用纯 vLLM raw-audio 推理。

| 参数 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `enable_hotword_recall` | bool | `true` | 是否启用 Triton 全局热词池召回。关闭后不注入热词，`enable_encoder_bypass` 会自动降级为 false |
