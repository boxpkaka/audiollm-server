...
接口说明见 [docs/transcription-jobs-api.md](docs/transcription-jobs-api.md)。推理用哪个模型、是否双模型融合由 `config.yaml` 的 `rest.transcribe` 块独立声明（省略则跟随通用 `rest.upstreams` 绑定），下表为 `defaults.transcribe` 调参：
