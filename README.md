...
- 可选：Triton 热词召回服务（默认 `http://localhost:10001` / `rag_asr_retrieve`），用于按 `user_id` 隔离的 10 万级用户热词池召回
...
| **热词** | Triton 按用户热词池召回 top-K，召回词注入 ASR prompt；1.7B 可走 encoder bypass |
...
| `{"type": "update_hotwords", "hotwords": ["词1", "词2"]}` | 更新后续 final 段的临时请求热词；去重限量后追加到 Triton 召回结果，也可继续用于携带 `enrollment_id` |
...
- `user_id` — 可选的热词池隔离 ID，默认 `default`；不同用户只管理和召回自己的 Triton 热词池
- `hotwords` — 临时请求热词。服务端不会把它写入用户池；final 段会把去重后的前 `recall_custom_hotword_limit` 个追加到 Triton 用户热词池召回结果后进入 prompt
...
ASR 热词偏置来自 `config.yaml -> services.recall` 指向的 Triton 用户热词池。客户端可显式传 `user_id`，未传时使用 `recall_user_id`（默认 `default`）；final 段只在该用户池内召回 top-K 热词，再追加少量请求临时 `hotwords`，最后写入主 ASR prompt。当主模型为 `amphion_asr_1.7b` 且未启用目标说话人时，服务端还会把 Triton 返回的 projector 帧作为 `audio_embeds` 发送给 vLLM，避免重复跑音频 encoder。伪流式 partial 不执行召回、不注入热词、也不走 encoder bypass，只使用纯 vLLM raw-audio 推理。

| 参数 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `enable_hotword_recall` | bool | `true` | 是否启用 Triton 用户热词池召回。关闭后不查用户池，主 ASR 只使用请求携带的 `hotwords`，`enable_encoder_bypass` 会自动降级为 false |
| `recall_top_k` | int | `50` | final 段召回并注入 prompt 的热词数量 |
| `recall_user_id` | string | `default` | 未显式传 `user_id` 时使用的热词池用户 ID |
| `recall_custom_hotword_limit` | int | `8` | 每次请求最多追加多少个临时 `hotwords` 到 prompt；去重、不写入用户池 |
