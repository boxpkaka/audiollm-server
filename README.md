...
- `hotwords` — 临时请求热词。服务端不会把它写入用户池；final 段会把去重后的前 `recall_custom_hotword_limit` 个优先注入 prompt，并覆盖精确重复或整词同音（忽略声调）的 Triton 召回热词
